...
module "vpc" {
  ...
  azs             = local.azs
  public_subnets  = [for index, _az in local.azs : cidrsubnet(var.vpc_cidr, 8, index)]
  private_subnets = [for index, _az in local.azs : cidrsubnet(var.vpc_cidr, 8, index + 10)]
...
resource "aws_secretsmanager_secret" "postgres" {
  ...
  description             = "PostgreSQL password for the NAGP assignment demo."
...
resource "aws_iam_policy" "external_secrets" {
  ...
  description = "Allow External Secrets Operator to read the assignment database secret."
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
